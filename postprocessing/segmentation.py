    ...
    def get_instances(self, impl: str = "skimage") -> Union[Tensor, Tuple[Tensor, ...]]:
        ...
        if impl == "skimage":
            if self.batch_size == 1:
                if self.cuda:
                    return NucleiInstances.from_seg_mask(self.mask.cpu()).as_tensor().cuda()
                else:
                    return NucleiInstances.from_seg_mask(self.mask).as_tensor()
            else:
                if self.cuda:
                    return tuple(NucleiInstances.from_seg_mask(mask.cpu()).as_tensor().cuda() for mask in self.mask)
                else:
                    return tuple(NucleiInstances.from_seg_mask(mask).as_tensor() for mask in self.mask)
