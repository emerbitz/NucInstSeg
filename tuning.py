...
from models.al_net import ALNet, ALNetDualDecoder
from models.att_net import AttentionUNet, ParaAttUNet
...
def tune_pprocess_params(trial):
    mode = "yang"
    # net = REUNet(mode=mode, net_params={"base_channels": 96, "base_channels_factor": 2, "aspp_inter_channels": 96})
    # net = ALNetLight(mode="graham", net_params={"aspp": False})
    # net = ALNetDualDecoder(mode=mode)
    net = ALNetLightDualDecoder(mode=mode)
    ckpt_name = "yang_alnetlightdual_lr=reu200"
    # ckpt_file = "top_pq_epoch=125.ckpt"
    ckpt_file = "top_val_epoch=96.ckpt"

    project = "al_net"
    log_dir = Path("lightning_logs", "tuning", "pprocess_params", f"{project}_{mode}")
    ckpt_dir = Path("trained_models", project, ckpt_name, ckpt_file)
    pprocess_params = suggest_pprocess_params(trial, mode=mode)
    log_name = str(pprocess_params)

    pl.seed_everything(42, workers=True)  # Seed for torch, numpy and python.random
    trainer = pl.Trainer(
        accelerator="gpu", devices=1,
        logger=TensorBoardLogger(save_dir=log_dir, name=log_name, default_hp_metric=False)
    )
    model = NetModel(net=net, pprocess_params=pprocess_params)
    ...
    trainer.test(model, data_module, ckpt_path=str(ckpt_dir), verbose=False)
    return trainer.logged_metrics["PQ"]
...
def tune_train_params(trial):
    ...
    model = NetModel(net=net, train_params=train_params, pprocess_params=None)
    trainer = pl.Trainer(
...
def tune_net_params(trial):
    ...
    model = NetModel(net=REUNet(mode=mode, net_params=net_params))
