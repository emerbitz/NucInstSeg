...
from postprocessing.dynamic_watershed_ import DynamicWatershedAlias
import numpy as np
import torch
...
class NucleiInstances:
    ...
    # @staticmethod
    # def from_seg_mask(seg_mask: Tensor, dilate: bool = False) -> "NucleiInstances":
    ...
    #                 if dilate:  # Maybe not optimal for model output
    #                     nucleus = dilation(nucleus, square(3))  # Morphological dilation restores nucleus contour
    ...
    def from_mask(seg_mask: Tensor, cont_mask: Tensor = None) -> "NucleiInstances":
        ...
        if cont_mask is not None:
            ...
            mask = seg_mask * np.logical_not(cont_mask)  # faster than np.logical_and(seg_mask, np.logical_not(cont_mask))
        else:
            mask = seg_mask
    ...
    def from_dist_map(dist_map: Tensor, param: int, thresh: Union[int, float]) -> "NucleiInstances":
        """Extracts nuclei instances from a distance map via dynamic watershed (Naylor et al. 2019)."""
        ...
        # labels = post_process(dist_map, param=param, thresh=thresh)
        labels = DynamicWatershedAlias(dist_map, param, thresh)
        return NucleiInstances.from_labeled_inst(labels)
    ...
    def to_dist_map(self) -> np.ndarray:
        ...
        for inst in self.nuc_inst:
            ...
            # else:
            #     map += dist
            map[inst] = dist[inst]  # Caution: Causes information loss if nuclei instances overlap!
        return map

    def to_hv_map(self) -> np.ndarray:  # -> Tuple[np.ndarray, np.ndarray]:
        """
